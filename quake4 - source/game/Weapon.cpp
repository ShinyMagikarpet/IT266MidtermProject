...
void rvWeapon::InitDefs( void ) {
	...
	// Super projectile -Dembner
	attackSuperDict.Clear();
	if (spawnArgs.GetString("def_superprojectile", "", &name) && *name) {
		def = gameLocal.FindEntityDef(name, false);
		if (!def) {
			gameLocal.Warning("Unknown super projectile '%s' for weapon '%s'", name, weaponDef->GetName());
		}
		else {
			spawnclass = def->dict.GetString("spawnclass");
			cls = idClass::GetClass(spawnclass);
			if (!cls || !cls->IsType(idProjectile::GetClassType())) {
				gameLocal.Warning("Invalid spawnclass '%s' for super projectile '%s' (used by weapon '%s')", spawnclass, name, weaponDef->GetName());
			}
			else {
				attackSuperDict = def->dict;
			}
		}
	}
...
void rvWeapon::AttackSuper(int num_attacks, float spread, float fuseOffset, float power) {
	idVec3 muzzleOrigin;
	idMat3 muzzleAxis;
