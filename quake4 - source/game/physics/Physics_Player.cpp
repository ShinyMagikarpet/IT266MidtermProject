...
const float PM_LADDERSPEED		= 200.0f; //-Dembner
...
void idPhysics_Player::LadderMove(void) {
	...
	//Move the player foward constantly -Dembner
	wishvel = viewForward * 300.0f; //Needs tweaking
	if (IsCrouching()) { //Strangely can still crouch when on "ladder". Allows easier use to bind brakes to crouch
		wishvel = viewForward * 50.0f;
		gameLocal.Printf("slower");
	}else if (command.upmove){//Boost for the player
		wishvel = viewForward * 800.0f;
		gameLocal.Printf("faster");
	}
	wishspeed = 50.0f;//blah blah
	idPhysics_Player::Accelerate(wishvel, wishspeed, 50.0f);//blah blah

	// stick to the ladder
	
	wishvel += -ladderNormal; //-100.0f * ladderNormal
	current.velocity = (gravityNormal * current.velocity) * gravityNormal + wishvel; //(-gravityNormal) -Dembner
	...
	if ( command.rightmove ) {
		...
		wishvel += 2.0f * right * scale * (float) command.rightmove; //2.0f -Dembner
	}
	...
	// do strafe friction
	//idPhysics_Player::Friction(); -Dembner
...
// RAVEN END
		...
		// walking on ground
		idPhysics_Player::LadderMove();//WalkMove(); -Dembner
		
	}
	else {
		// airborne
		idPhysics_Player::LadderMove();//AirMove(); -Dembner
	}
